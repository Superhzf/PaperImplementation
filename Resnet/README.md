...
As a result, the `20-layer` model on this implementation can see `90.51%` accuracy (vanilla residule block) on the test vs `91.25%` reported by the paper. The one uses the bottleneck block returns `91.34%`. If normalizing the dataset using the accurate mean and std (see details below), probably it will further improve the accuracy.
